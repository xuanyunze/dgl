...
public class DEVCMFA60Impl implements DeviceInterface{
	...
	public void installKeys(String keystr, int keytype) {		
		if(keytype==0){
			...
			System.out.println(this.deviceKeys);
			System.out.println(this.deviceKeys.substring(200,216));
	...
	public void initDevice(final Context context, final String address,boolean isEntrack) {
		...
		new Thread(new Runnable(){
			@Override
			public void run() {
				Controler.Init(context, CONNECTMODE.AUDIO, 0);
				ConnectPosResult info = Controler.connectPos("");
				if(info.bConnected){
	...
	public void encryptPwd(final EmvReadCardInfoVo cinfo,final String pwd) {
		new Thread(new Runnable(){
			...
			public void run() {
				try{
					deviceListener.showMessage("正在计算交易数据...");
					System.out.println("uuu=="+pwd);
					System.out.println("kkk=="+cinfo.getCardNo());
					if(StringUtil.isDbNull(cinfo.getCardNo())){
						deviceListener.showMessage("交易错误[芯片卡读卡方式错误～]");
					}else{
						CalPinResult result = Controler.CalPin(pwd.getBytes(),cinfo.getCardNo());
						if(result.commResult == (CommEnum.COMMRET.NOERROR)){
							cinfo.setPinData(CodeUtil.byte2HexStr(result.pinBlock));
							System.out.println("xxxxx==="+CodeUtil.byte2HexStr(result.pinBlock));
							deviceListener.onFinishComputepwd(cinfo);
						}else{
							deviceListener.showMessage("密码计算错误...");
						}
					}
				}catch(Exception e){
					deviceListener.showMessage("密码计算错误...");
